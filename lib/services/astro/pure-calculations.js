...
function calculatePlanetPositions(jd) {
    return new Promise((resolve) => {
        ...
        // Calculate houses first to get MC and ASC
        calculateHouses(jd, 0, 0).then(houses => {
            ...
            const positions = {
                ...
                NorthNode: calculateNorthNodePosition(T),
                Chiron: calculateChironPosition(T),
                ASC: {
                    longitude: houses.Ascendant.cusp,
                    latitude: 0,
                    distance: 1.0,
                    longitude_speed: 0,
                    sign: houses.Ascendant.sign,
                    formatted: houses.Ascendant.formatted
                },
                MC: {
                    longitude: houses.Midheaven.cusp,
                    latitude: 0,
                    distance: 1.0,
                    longitude_speed: 0,
                    sign: houses.Midheaven.sign,
                    formatted: houses.Midheaven.formatted
                }
            }
...
function calculateNorthNodePosition(T) {
    // Mean longitude of the Moon's ascending node
    let N = 125.0445550 - 1934.1361849 * T + 0.0020762 * T * T + T * T * T / 467410.0 - T * T * T * T / 60616000.0
    N = ((N % 360) + 360) % 360
    ...
    return {
        ...
        latitude: 0,
        distance: 1.0,
        longitude_speed: -0.0529539,
        sign: getZodiacSign(N),
        formatted: formatDegreeMinute(N)
    }
}

// Chiron calculation (simplified)
function calculateChironPosition(T) {
    // Approximate Chiron's position
    const meanLongitude = 50.077 + 43.2567 * T
    const longitude = ((meanLongitude % 360) + 360) % 360
    
    return {
        longitude,
        latitude: 0,
        distance: 1.0,
        longitude_speed: 0.0833333,
        sign: getZodiacSign(longitude),
        formatted: formatDegreeMinute(longitude)
    }
}
