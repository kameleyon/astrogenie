...
function Calendar({
  ...
  return (
    <div className="relative w-full max-w-md">
      ...
      <input
        type="text" // Changed to text to remove default date picker
        value={selectedDate ? format(selectedDate, 'MM/dd/yyyy') : ''}
        onFocus={() => setShowCalendar(false)}
        onChange={() => {}}
        placeholder="Your birth date"
        className="w-full h-10 px-10 py-2 text-white/70 bg-black/20 border border-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FFA600]/50"
      />

      {/* Custom calendar icon positioned before the input text */}
